...
def get_smote_standard(data_list, label):
    smo = SMOTE(random_state=42)
    X_smo, y_smo = smo.fit_sample(data_list, label)
    return X_smo, y_smo

def get_smoteenn(data_list, label):
    smo = SMOTEENN(random_state=42)
    X_smo, y_smo = smo.fit_resample(data_list, label)
    return X_smo, y_smo 
   
def get_smotetomek(data_list, label):
    smo = SMOTETomek(random_state=42)
    X_smo, y_smo = smo.fit_resample(data_list, label)
    return X_smo, y_smo   


def get_smote_borderline_1(clf, data, label, m, s, k=5):
    label_local = label[:]
